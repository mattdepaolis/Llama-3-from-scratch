...
#from sentencepiece import SentencePieceProcessor  # Importing SentencePieceProcessor for tokenization
...
class Attention(nn.Module):
    ...
    ###Model Features
    RMS-Normalization: RMSNorm is a simplification of the original layer normalization (LayerNorm). 
    LayerNorm is a regularization technique that might handle the internal covariate shift issue 
    so as to stabilize the layer activations and improve model convergence. 
    It has been proved quite successful in LLaMA 2.

    Activation Function: 
    LLaMA 2 uses the SwiGLU activation function instead of ReLU, leading to improved training performance.

    Rotary Positional Embeddings (RoPE): 
    Inspired by the GPT-Neo-X project, LLaMA 2 incorporates rotary positional embeddings at each layer, 
    enhancing the model's positional understanding.

    Increased Context Length and Grouped-Query Attention (GQA): 
    LLaMA 2 model has a doubled context window (from 2048 to 4096 tokens) and employs grouped-query attention. 
    This allows for better processing of long documents, chat histories, and summarization tasks.

    ###Rotary Positional Encoding (RoPE)
    The inclusion of RoPE is particularly noteworthy. Traditional positional encodings add 
    or concatenate position information to input embeddings before processing them through the model. 
    In contrast, RoPE integrates positional information directly into the self-attention mechanism, 
    allowing the model to preserve the relative positional information of tokens through the layers.
    This approach can lead to improvements in the model's understanding of sequence order, which is crucial 
    for many tasks in natural language processing and beyond.

    Flexible Attention Mechanism
    The Attention class itself is designed with flexibility in mind. By separating the number of 
    attention heads (n_heads) from the number of key/value heads (n_kv_heads), and introducing the concept 
    of repeats, the model can potentially explore different configurations of distributing attention across 
    the input sequence. This flexibility might allow for more efficient learning by focusing 
    the model's attention differently based on the task at hand.

    ###Parameter Efficiency
    The approach to key, query, and value projections, as well as the output projection, 
    through linear transformations is a testament to the efficiency of the design. 
    The separation of dimensions for these projections allows the model to control its capacity and 
    the granularity of its attention mechanism. Additionally, by utilizing linear layers without bias 
    for these transformations, the model emphasizes the importance of the learned weights, possibly leading 
    to a more interpretable attention mechanism.

    ###Modularity and Integration
    The explicit definition of RoPE within the Attention class highlights the modular design approach. 
    This modularization not only makes the code more readable and maintainable but also facilitates 
    the experimentation with or replacement of positional encoding strategies without affecting 
    the core attention mechanism.

    ###Handling of Key/Value Heads (Grouped-Query Attention)
    One of the unique aspects of this implementation is its approach to managing the number of 
    attention heads (n_heads) and the number of key/value heads (n_kv_heads). By allowing these to differ 
    and introducing a mechanism to repeat key/value pairs across attention heads (self.repeats), 
    the model can potentially increase its expressiveness and efficiency. This feature allows for 
    a more flexible distribution of computational resources across different components of 
    the attention mechanism, potentially leading to improved learning dynamics and model performance.

    ###Efficient Computation and Caching
    The __call__ method, serving as the forward pass, showcases efficient computation strategies, 
    including reshaping and transposing for multi-head attention processing and the conditional use 
    of caching for recurrent computations. This design facilitates the reuse of computations, 
    crucial for tasks requiring sequential data processing, like language modeling, where dependencies 
    span long sequences.
    """
...
def generate(args):
    ...
    x = mx.array([[tokenizer.bos_id] + tokenizer.encode(args.prompt, bos=False, eos=True)])
    #x = mx.array([tokenizer.encode(args.prompt, bos=False, eos=False)])
...
def few_shot_generate(args):
    """
    Function to generate text using few-shot prompts.
    ...
    # Sub-function to determine if a string ends with a specific pattern
    def possible_end(s):
        word = "[Instruction]"
        # Iterate backwards through the word to check if the end of the string matches any part of the word
        for i in range(len(word) - 1, 0, -1):
            if s[-i:] == word[:i]:
                return 0
        # Check if the string ends with the word "[Instruction]"
        if s[-len(word):] == word:
            return 1
        # Return -1 if the string does not end with the word or any part of it
        return -1

    # Sub-function to generate text based on a given question
    def generate(question):
        # Preprocess the question for the model
        x = mx.array([[tokenizer.bos_id()] + tokenizer.encode(question)])
        skip = 0  # To track how much of the generated string should be skipped in the output
        prompt_processing = None  # To track prompt processing time
        tokens = []  # List to collect generated tokens
        start = tic()  # Start time for performance measurement

        # Generate tokens from the model
        for token in model.generate(x, args.temp):
            tokens.append(token)

            # Measure prompt processing time after the first token is generated
            if len(tokens) == 1:
                mx.eval(token)
            ...
            # Evaluate the generated tokens for any further processing needed
            mx.eval(tokens)
            token_list = [t.item() for t in tokens]
            s = tokenizer.decode(token_list)

            # Determine if the generated text should end
            end = possible_end(s)
            if end == 0:
                continue
            if end == 1:
            ...
            # Print the generated text, skipping the already printed part
            print(s[skip:], end="", flush=True)
            skip = len(s)
            # Break if the end of sentence token is generated
            if token_list[-1] == tokenizer.eos_id():
                break

        # Evaluate the full generation for performance measurement
        mx.eval(tokens)
        full_gen = toc("Full generation", start)
        s = tokenizer.decode([t.item() for t in tokens])
        print(s[skip:], end="", flush=True)

    # Load few-shot examples and replace placeholders with actual questions
...
def load_model(model_path):
    ...
    if unsharded_weights_path.is_file():
        ...
        # Assuming `weights` is your original dictionary
        #weights = {key: value for key, value in weights.items() if 'layers.21' not in key}
        #weights = {key: value for key, value in weights.items() if 'layers.21' not in key and 'layers.20' not in key}
        #weights = {key: value for key, value in weights.items() if 'layers.21' not in key and 'layers.20' not in key and 'layers.19' not in key}
